...
<?php
	...
	if($_POST["addflag"] == "add"){
			...
			$stid2 = oci_parse($conn, "begin Database.addSpec(:p_stuId,:p_deptId,:p_name,:p_surname,:p_natId,:p_email,:p_phone,:p_address,:p_password,:p_pre,:p_RESULT); end;");
	...
	$stid = oci_parse($conn, "begin Database.showSpec(:p_profile); end;");
	...
	<center><h2>Special Student </h2></center>
...
<center>	
		...
		<form action="addSpec.php" method="POST">
			...
			<p><label for="pre">Institution:</label><input type="text" name="pre" value="" required></p>
