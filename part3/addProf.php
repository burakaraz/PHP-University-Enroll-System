...
<?php
	...
	if($_POST["addflag"] == "add"){
			...
			$stid2 = oci_parse($conn, "begin Database.addProf(:p_stuId,:p_deptId,:p_name,:p_surname,:p_natId,:p_salary,:p_email,:p_phone,:p_address,:p_password,:p_title,:p_office,:p_pre,:p_RESULT); end;");
			...
			oci_bind_by_name($stid2, ":p_title", $title);
			oci_bind_by_name($stid2, ":p_office", $office);
			oci_bind_by_name($stid2, ":p_pre", $pre);
...
<center>	
		...
		<form action="addProf.php" method="POST">
			...
			<p><label for="title">Title:</label><input type="text" name="title" value="" required></p>
			<p><label for="office">Office:</label><input type="text" name="office" value="" required></p>
			<p><label for="pre">Graduate School:</label><input type="text" name="pre" value="" required></p>
