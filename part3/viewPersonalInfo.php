...
<?php
	...
	if(isset($_POST['address']) || isset($_POST['email']) || isset($_POST['phoneNo'])) {
		...
		$stid = oci_parse($conn, "begin Database.updateProfile(:p_userID,:p_condition,:p_address,:p_phone,:p_email,:p_RESULT); end;");
	...
	$stid = oci_parse($conn, "begin Database.getProfile(:p_userID,:p_condition,:p_profile); end;");
	...
	<center>
		<p><label for="studID">StudentID: <?php echo $row[0]; ?></label></p>
		...
		<form action="viewPersonalInfo.php" method="POST">
			...
			<p><label for="address">Address:</label> <input type="text" name="address" value="<?php echo $row[7]; ?>"></p>
			<p><label for="phoneNo">Phone Number:</label> <input type="text" name="phoneNo" value="<?php echo $row[6]; ?>"></p>
			<p><label for="email">Email:</label> <input type="text" name="email" value="<?php echo $row[5]; ?>"></p>
