...
<?php
	...
	if(isset($_POST['address']) || isset($_POST['email']) || isset($_POST['phoneNo'])) {
		...
		$stid = oci_parse($conn, "begin Database.updateProfProfile(:p_userID,:p_address,:p_phone,:p_email,:p_RESULT); end;");
	...
	$stid = oci_parse($conn, "begin Database.getProfProfile(:p_userID,:p_profile); end;");
	...
	<center>
		...
		<p><label for="title">Title: <?php echo $row[11]; ?></label></p>
		<p><label for="office">Office: <?php echo $row[12]; ?></label></p>
		<p><label for="grad">Grad School: <?php echo $row[13]; ?></label></p>
		<br><br>	
